...
function f_c_i(table, r_num, c_num){
    return table.rows[r_num].cells[c_num].value;
}
...
function build_table(tableid, array, type){
    ...
    while (row_count <= array.length){
            console.log(Object.values(array[row_count])[0]);
...
function con_table(tableid, type, key){
    var newarray= [];
    var row_count = 0;
...
// c_content is content of the cell, type indicates if it is first or normal
// function table_row(r_content, type){
//     if(type == "first"){
//     return "<tr><td><input type ='text' value="+c_content+"></input>"+"</td><td>";}
//     else if (type == "normal"){
//     return + "</td><td><input type ='text' value="+c_content+"></input>"
//     }
